...
public class OcclusionHelper {
    ...
    public static boolean testPartIntersection(IPartInfo part1, IPartInfo part2) {
        return part1.getPart().testIntersection(part1.getWorld(), part1.getPos(), part1, part2)
                || part2.getPart().testIntersection(part2.getWorld(), part2.getPos(), part2, part1);
    }
    ...
    public static boolean testContainerBoxIntersection(IBlockAccess world, BlockPos pos, Collection<AxisAlignedBB> boxes, Predicate<IPartSlot> ignore) {
    ...
    public static boolean testContainerBoxIntersection(IMultipartContainer container, Collection<AxisAlignedBB> boxes, Predicate<IPartSlot> ignore) {
        return testBoxIntersection(container.getParts().values().stream().filter(i -> !ignore.test(i.getSlot()))
                .map(i -> i.getPart().getOcclusionBoxes(i.getWorld(), i.getPos(), i)).flatMap(List::stream).collect(Collectors.toList()),
                boxes);
    }
