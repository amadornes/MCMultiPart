...
public interface IMultipartContainer extends IWorldLocation, ISlottedCapabilityProvider
{
    ...
    /**
     * Adds a part to the container. You MUST check if
     * {@link IMultipartContainer#canAddPart(IMultipart)} or
     * {@link IMultipartContainer#canReplacePart(IMultipart, IMultipart)} return
    ...
    /**
     * Gets the UUID of a specific part. Used to synchronize the server and the
     * client.
     */
    ...
    /**
     * Adds a part with a specific UUID. Used when adding a part on the
     * clientside through a placement packet. Do not call this manually.
     */
    public void addPart(UUID id, IMultipart part);
    
    /**
     * Performs an occlusion test against all the parts in this container except
     * for the specified ones, and possibly tests against the container itself.
     */
