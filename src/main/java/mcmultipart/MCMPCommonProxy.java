    ...
    public void onPlayerRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (!stack.isEmpty()) {
            ...
            if (info != null && info.getKey().getBlockPlacementLogic() != null) {
                EnumActionResult result = placePart(stack, player, event.getWorld(), event.getPos(), event.getFace(), (float) event.getHitVec().xCoord,
                        (float) event.getHitVec().yCoord, (float) event.getHitVec().zCoord, event.getHand(), info);
